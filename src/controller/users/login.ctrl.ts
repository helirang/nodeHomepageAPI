...
const login = async function(req:express.Request,res:express.Response){
    let query = 'SELECT id,pw,salt FROM user WHERE id=?';
    ...
    if(data[0].pw == checkPw) {
        ...
        res.cookie('id',data[0].id,{maxAge:(60*60*24*30)})
    }else{
